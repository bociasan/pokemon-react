...
export const Multipliers = (props) => {
  ...
  useEffect(() => {
    if (
      countSelectedTypes(defenceTypes) > 0 &&
      countSelectedTypes(attackTypes) > 0
    ) {
  ...
  const firstRowOnClick = (type) => {
    // console.log(type)
    if (
      countSelectedTypes(defenceTypes) < MAX_TYPES_SELECTED ||
      defenceTypes[type]
    ) {
  ...
  const firstColumnOnClick = (type) => {
    // console.log(type)
    if (
      countSelectedTypes(attackTypes) < MAX_TYPES_SELECTED ||
      attackTypes[type]
    ) {
  ...
  let firstRow = Object.keys(types).map((el2) => (
    <td
      onClick={() => firstRowOnClick(el2)}
      key={el2 + el2}
      className={
        defenceTypes[el2]
          ? "multipliers-table-cell first-row bold selected-defence"
          : "multipliers-table-cell first-row bold"
      }
    >
      {el2}
    </td>
  ...
  firstRow.unshift(
    <td
      className="multipliers-table-cell"
      onClick={() => pokeballOnClick()}
      style={{ borderWidth: 0 }}
    >
    ...
    <tr className="multipliers-table-row">
      {el1 === "first"
        ...
        : first_row_keys.map((el2) =>
            el2 === "first" ? (
              <td
                onClick={() => firstColumnOnClick(el1)}
                className={
                  attackTypes[el1]
                    ? "multipliers-table-cell first-column bold selected-attack"
                    : "multipliers-table-cell first-column bold"
                }
              >
                {el1}
              </td>
  ...
  return (
    <div className="multipliers-page-div">
